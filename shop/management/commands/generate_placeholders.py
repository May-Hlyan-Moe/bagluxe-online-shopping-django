...
def split_name(name):
    words = name.split()
    mid = len(words) // 2
    line1 = ' '.join(words[:mid]) if mid else name
...
class Command(BaseCommand):
    help = 'Generate branded SVG placeholder images for all products'
    ...
    def handle(self, *args, **kwargs):
        ...
        for idx, product in enumerate(products):
            ...
            self.stdout.write(f'  Generated: {product.brand.name} - {product.name}')

        self.stdout.write(self.style.SUCCESS(f'\nDone! {products.count()} product images generated.'))
